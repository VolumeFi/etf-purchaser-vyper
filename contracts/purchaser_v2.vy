...
event CreateCompositeETF:
    ...
    etf_token_description: String[256]
    expense_ratio: uint256
    token_denoms: DynArray[String[96], 25]
    token_positions: DynArray[uint256, 25]
...
@payable
@nonreentrant
def create_composite_etf(_etf_token_name: String[64], _etf_token_symbol: String[32], _etf_token_description: String[256], _expense_ratio: uint256, _token_denoms: DynArray[String[96], 25], _token_positions: DynArray[uint256, 25]):
    assert _etf_token_name != "", "Invalid token name"
    assert _etf_token_symbol != "", "Invalid token symbol"
    assert _expense_ratio <= 1000000, "Invalid expense ratio"
    assert self.fee_receiver != empty(address), "Fee receiver not set"

    assert len(_token_denoms) > 1, "At least 2 tokens required"
    assert len(_token_denoms) == len(_token_positions), "length mismatch"

    _total_position: uint256 = 0
    for _position: uint256 in _token_positions:
        assert _position > 0, "Invalid token position"
        _total_position += _position
    assert _total_position == 100, "sum must to be 100"
...
@payable
@nonreentrant
def register_composite_etf(_etf_token_denom: String[96], _etf_token_name: String[64], _etf_token_symbol: String[32], _etf_token_description: String[256], _expense_ratio: uint256, _token_denoms: DynArray[String[96], 25], _token_positions: DynArray[uint256, 25]):
    assert _etf_token_denom != "", "Invalid ETF token address"
    assert _etf_token_name != "", "Invalid ETF token name"
    assert _etf_token_symbol != "", "Invalid ETF token symbol"
    assert _expense_ratio <= 1000000, "Invalid expense ratio"
